...
function PutFiles() {
	...
	for (i = 0; i < listCheckedBox.length; i++) {
		if (listCheckedBox[i].checked == true) {
			var vals = listCheckedBox[i].value.split("?");
			var change = {};
			change.ETA = listCheckedBox[i].getAttribute("etalon");
			change.BST = vals[0].replace(/"/g, "");
			change.NEW = vals[1].replace(/"/g, "");
			changes.push(change);
		}
	}
	...
	if (changes.length < 1) {
		...
		$.ajax({
			type: "POST",
			url: "WebService.asmx/ProcessNewEtalons",
			data: JSON.stringify({ files: changes, branch: gitBranch }),
